...
document.getElementById('start-wp-btn').onclick = async () => {
    const url = document.getElementById('wp-link-input').value;
    const vid = extractVideoID(url);
    if(!vid) return showToast("ลิงก์ YouTube ไม่ถูกต้อง", "error");
    await setDoc(doc(db, "appData", "watchParty"), { videoId: vid, state: 1, time: 0, updatedBy: currentUsername, timestamp: serverTimestamp() });
    document.getElementById('wp-link-input').value = ""; document.getElementById('wp-controls').classList.add('hidden');
    showToast("เริ่มการสตรีมคลิป YouTube!", "success");
};
document.getElementById('close-wp-btn').onclick = async () => {
    if(confirm("ต้องการปิดคลิปสำหรับทุกคนในห้องใช่ไหม?")) {
        await setDoc(doc(db, "appData", "watchParty"), { videoId: "", state: -1, time: 0, updatedBy: currentUsername, timestamp: serverTimestamp() });
        showToast("ปิดการสตรีมคลิปเรียบร้อย", "success");
    }
};
...
async function onPlayerStateChange(event) {
    ...
    if(event.data === 1 || event.data === 2) {
        ...
        if(ytPlayer && typeof ytPlayer.getCurrentTime === 'function') {
            ...
            await setDoc(doc(db, "appData", "watchParty"), { videoId: vid, state: event.data, time: time, updatedBy: currentUsername, timestamp: serverTimestamp() });
        }
    }
}
...
function initOrUpdatePlayer(vid, time, state, host) {
    ...
    if(!ytPlayer) {
        ...
        if(currentVid !== vid) {
            ignoreNextYtEvent = true; ytPlayer.loadVideoById(vid, time);
        } else {
            if(host !== currentUsername) {
                ignoreNextYtEvent = true; const currentTime = ytPlayer.getCurrentTime();
                if(Math.abs(currentTime - time) > 2) ytPlayer.seekTo(time, true);
                if(state === 1 && ytPlayer.getPlayerState() !== 1) ytPlayer.playVideo();
                if(state === 2 && ytPlayer.getPlayerState() !== 2) ytPlayer.pauseVideo();
            }
        }
    }
}
...
function updateUnreadBadge(channel) {
    const btn = document.querySelector(`.nav-btn[data-channel="${channel}"]`); if (!btn) return;
...
window.showUserProfile = (userName) => {
    ...
    document.getElementById('profile-card-name').textContent = userName; document.getElementById('profile-card-avatar').src = u.avatar;
    ...
    else badges += '<span class="bg-[#5865F2]/20 text-[#5865F2] px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border border-[#5865F2]/30">Member</span>';
...
document.getElementById('close-profile-card').onclick = () => document.getElementById('profile-card-modal').classList.add('hidden');
document.getElementById('profile-card-modal').addEventListener('click', (e) => { if (e.target === document.getElementById('profile-card-modal')) document.getElementById('profile-card-modal').classList.add('hidden'); });
...
function renderMessages() {
    ...
    filteredMessages.forEach((m) => {
        ...
        if (m.senderName === "🤖 System Bot") {
            chatContainer.insertAdjacentHTML('beforeend', `<div class="chat-msg-row flex space-x-3 md:space-x-4 hover:bg-[#151619]/60 px-2 md:px-4 py-3 mt-4 -mx-2 md:-mx-4 group transition duration-150 relative items-center border-l-[3px] border-transparent hover:border-[#5865F2]"><div class="w-8 md:w-10 h-8 md:h-10 rounded-full bg-[#5865F2]/10 flex items-center justify-center flex-shrink-0 border border-[#5865F2]/20"><i class="ph-fill ph-robot text-[#5865F2] text-[20px] md:text-[24px]"></i></div><div class="min-w-0 flex-1 pb-1"><div class="flex items-baseline space-x-2"><span class="font-extrabold text-[12px] text-white bg-[#5865F2] px-1.5 py-0.5 rounded uppercase tracking-wider">System</span><span class="text-[10px] md:text-[11px] text-[#6d717a]">${timeString}</span></div><p class="text-[#949ba4] mt-1 text-[13px] md:text-[14px] leading-relaxed">${formattedText}</p></div></div>`);
        ...
        if (lastSender === m.senderName && !isReply) {
            ...
            chatContainer.insertAdjacentHTML('beforeend', `<div class="chat-msg-row flex flex-col hover:bg-[#151619]/60 px-2 md:px-4 py-2 mt-4 -mx-2 md:-mx-4 group transition duration-150 relative">${replyBlockHTML}<div class="flex space-x-3 md:space-x-4"><img src="${msgAvatarUrl}" onclick="showUserProfile('${m.senderName}')" class="w-8 h-8 md:w-10 md:h-10 rounded-full flex-shrink-0 object-cover opacity-95 cursor-pointer hover:opacity-80 transition"><div class="min-w-0 flex-1 pb-1 pr-20"><div class="flex items-baseline space-x-2"><span onclick="showUserProfile('${m.senderName}')" class="font-medium text-[14px] md:text-[15px] text-[#e4e5e7] tracking-wide hover:underline cursor-pointer">${m.senderName}</span><span class="text-[10px] md:text-[11px] text-[#6d717a]">${timeString}</span></div>${contentHTML}${reactsHTML}</div></div>${actionMenuUI}</div>`);
        }
...
// 🌟 ระบบ Slash Commands ทอยเต๋า/ล้างแชท
...
document.querySelectorAll('#command-list li').forEach(li => {
    li.onclick = () => { chatInput.value = li.getAttribute('data-cmd') + " "; chatInput.focus(); cmdMenu.classList.add('hidden'); }
});
...
async function sendMessage() { 
    ...
    // ตรวจสอบว่าเป็นคำสั่งบอทหรือไม่
    if(txt.startsWith('/')) {
        let botReply = "";
        if(txt === '/roll') {
            const roll = Math.floor(Math.random() * 100) + 1;
            botReply = `🎲 **${currentUsername}** ทอยลูกเต๋าได้แต้ม **${roll}** (จาก 100)`;
        } else if(txt === '/coin') {
            const coin = Math.random() < 0.5 ? "หัว" : "ก้อย";
            botReply = `🪙 **${currentUsername}** โยนเหรียญออก **${coin}**`;
        } else if(txt === '/clear') {
            if(currentUserRole !== 'Admin') { showToast("คุณไม่ใช่ Admin ไม่มีสิทธิ์ใช้คำสั่งนี้", "error"); return; }
            if(!confirm('🚨 ยืนยันการล้างแชททั้งหมดหรือไม่? (กู้คืนไม่ได้)')) return;
            const msgs = allMessages.filter(m => m.channel === activeChannel);
            for(let m of msgs) await deleteDoc(doc(db, "messages", m.id));
            botReply = `🧹 แอดมิน **${currentUsername}** ได้ทำการล้างห้องแชทเรียบร้อยแล้ว`;
        } else {
...
// ==========================================
// 🎨 8. ระบบกระดานไอเดีย (Shared Whiteboard)
// ==========================================
...
// ==========================================
// 🚀 11. ลงทะเบียน PWA (Progressive Web App) และ Onboarding Tour
// ==========================================
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('sw.js')
            .then(registration => console.log('✅ DOSH ServiceWorker ทำงานเรียบร้อย'))
            .catch(err => console.log('❌ DOSH ServiceWorker ลงทะเบียนไม่สำเร็จ:', err));
    });
}
...
const tourSteps = [
    { target: null, title: "ยินดีต้อนรับสู่ DOSH! 🎉", desc: "Super App สำหรับทีมครีเอทีฟและมัลติมีเดีย จบครบทุกงานในเว็บเดียว! เดี๋ยวเราจะพาไปดูว่ามีเครื่องมืออะไรให้ใช้บ้าง", pos: "center" },
    { target: ".nav-btn[data-view='chat']", title: "1. ห้องแชทอัจฉริยะ 💬", desc: "คุยงาน ส่งไฟล์ ซูมรูปภาพ พิมพ์คำสั่ง / บอท หรือแม้แต่ 'ตอบกลับข้อความ' ก็ทำได้ครบจบที่นี่!", pos: "right" },
    { target: ".nav-btn[data-view='voice']", title: "2. ห้องนั่งเล่น (Voice Lounge) 🎙️", desc: "เปิดไมค์คุยงาน แชร์หน้าจอ (Screen Share) หรือจะเปิดคลิป YouTube รัน Watch Party ดูพร้อมกันทั้งแก๊งก็ยังได้!", pos: "right" },
    { target: ".nav-btn[data-view='board']", title: "3. ระบบกระดานงาน (Task Board) 📋", desc: "สร้างงาน จัดหมวดหมู่ แล้วลากแปะ (Drag & Drop) เพื่ออัปเดตสถานะงานให้เพื่อนๆ ในทีมรู้ความคืบหน้าแบบ Real-time", pos: "right" },
    { target: "#mini-profile-btn", title: "4. ปรับแต่งโปรไฟล์ 🪪", desc: "กดตรงนี้เพื่อตั้งค่า 'รูปภาพปก (Banner)' เปลี่ยนสีชื่อ และตั้งสถานะ (Custom Status) โชว์ความเท่ให้เพื่อนในทีมเห็น!", pos: "top" }
];
...
function highlightElement(selector) {
    ...
    if (el) {
        ...
        activeHighlightTarget = el; const rect = el.getBoundingClientRect();
        tourTooltip.style.top = `${rect.top + (rect.height / 2) - (tourTooltip.offsetHeight / 2)}px`; tourTooltip.style.left = `${rect.right + 20}px`;
        if(window.innerWidth < 768 || rect.right + 340 > window.innerWidth) { tourTooltip.style.top = `${rect.bottom + 20}px`; tourTooltip.style.left = `20px`; }
    }
}
...
function showTourStep(index) {
    ...
    if (index === tourSteps.length - 1) { tourNextBtn.innerHTML = `เริ่มใช้งาน DOSH! <i class="ph-fill ph-rocket-launch ml-1.5"></i>`; tourNextBtn.classList.replace('bg-[#5865F2]', 'bg-[#23a559]'); tourNextBtn.classList.replace('hover:bg-[#4752C4]', 'hover:bg-[#1e8a49]'); } else { tourNextBtn.innerHTML = `ต่อไป <i class="ph-fill ph-caret-right ml-1.5"></i>`; }
    if (step.pos === "center") { tourTooltip.style.top = "50%"; tourTooltip.style.left = "50%"; tourTooltip.style.transform = "translate(-50%, -50%)"; highlightElement(null); } else { tourTooltip.style.transform = "none"; highlightElement(step.target); }
}
